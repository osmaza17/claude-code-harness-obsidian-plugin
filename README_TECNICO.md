...
### Fuente del % — API primero, scraping de respaldo (invertido el 2026-07-17)
El % de la cuenta activa se toma **primero de la API** (`usagePct()`, ver §5b):
es el dato autoritativo (las mismas cabeceras de rate-limit que alimentan la
barra), se refresca cada minuto para todas las cuentas y no depende del texto de
la TUI. Solo si no hay lectura fresca de la API (sondeo apagado, red caída,
token muerto) se cae al **scraping** de la barra con la regex configurable.
Peor caso del API-primero: la lectura puede tener hasta ~1 min de antigüedad
(el tick del sondeo), que el margen del 10 % del techo absorbe. El **anclaje por
email de la barra solo aplica al valor scrapeado** (la barra puede ir desfasada
tras un swap); la API ya está atada al token de la cuenta, así que no necesita
anclaje. Antes el orden era el inverso (scraping primero); se invirtió para que
la decisión no dependa de una regex frágil frente a cambios del CLI.
...
# APÉNDICE — Referencia exhaustiva del plugin (movida desde CLAUDE.md el 2026-07-17)

> Este apéndice es el antiguo CLAUDE.md completo, movido aquí íntegro para
> adelgazar el contexto que carga cada sesión de Claude. Contiene la narrativa
> detallada de cada subsistema, bug y decisión (el "porqué" largo). El CLAUDE.md
> actual conserva la versión operativa condensada y enlaza aquí.
